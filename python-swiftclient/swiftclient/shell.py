#!/usr/bin/python -u
# Copyright (c) 2010-2017 OpenStack, LLC.
#
...
from secdel import secdel_list_account, secdel_get_barbican_manager_and_ctxt, \
    secdel_list_barbican_secrets, secdel_create_barbican_secret, \
    secdel_delete_barbican_secret, secdel_get_account_key_id_by_parent_id, \
    secdel_get_barbican_secret, secdel_rekey, secdel_rewrap, \
    secdel_list_container, secdel_list_keys, secdel_purge, secdel_exists, \
    SECDEL_LOG_LEVEL_DEBUG, SECDEL_LOG_LEVEL_INFO, SECDEL_LOG_LEVEL_ERROR
...
commands = ('delete', 'download', 'list', 'post', 'copy', 'stat', 'upload',
            'capabilities', 'info', 'tempurl', 'auth', 'secdel', 'rekey')
...
def _perform_delete(swift, args, output_manager, options):
    try:
        if not args:
...
def st_delete(parser, args, output_manager):
    ...
    with SwiftService(options=options) as swift:
        _perform_delete(swift, args, output_manager, options)
...
st_secdel_options = '''[--root-uuid <uuid>]
'''

st_secdel_help = '''
Securely deletes an account, container, or object, by rekeying, rewrapping,
and purging encryption keys such that the account, container, or object
content is no longer decryptable.

Positional arguments:
  [<container>]           Name of container to securely delete.
  [<object>]              Name of object to securely delete.

Optional arguments:
  -u, --root-uuid <uuid>  Use the root secret specified by the given uuid for
                          completing the secure deletion operation. This
                          argument can be used to continue an interrupted
                          secure deletion operation.
'''.strip('\n')


def st_secdel(parser, args, output_manager):
    parser.add_argument(
        '-H', '--header', action='append', dest='header',
        default=[],
        help='Adds a custom request header to use for securely deleting '
        'objects or an entire container.')
    parser.add_argument(
        '-u', '--root-uuid',
        default=None, help='ID of root secret to use. For continuing an '
        'interrupted secure deletion operation. Default: None')
    parser.add_argument(
        '-a', '--all', action='store_true', dest='yes_all',
        default=False, help='Delete all containers and objects.')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Do not delete securely deleted objects or containers.')

    # Do not print out INFO messages about connections to Barbican, Keystone
    logging.getLogger('requests').setLevel(logging.WARNING)
    logging.getLogger('barbicanclient').setLevel(logging.CRITICAL)
    logging.getLogger('castellan.key_manager').setLevel(logging.CRITICAL)
    options, args = parse_args(parser, args)
    args = args[1:]

    root_uuid = options.get('root_uuid')
    container = None
    object = None
    if args is not None:
        logger.log(SECDEL_LOG_LEVEL_DEBUG, "len(args): %d" % len(args))
        if len(args) > 0:
            logger.log(SECDEL_LOG_LEVEL_DEBUG, "container is '%s'" % args[0])
            container = args[0]
        if len(args) > 1:
            logger.log(SECDEL_LOG_LEVEL_DEBUG, "object is '%s'" % args[1])
            object = args[1]
    else:
        logger.log(SECDEL_LOG_LEVEL_DEBUG, "args is None")
    # (Temporary) safeguard so the account does not get accidentally deleted
    if container is None and object is None:
        print("The account cannot be securely deleted using the swift client.")
        print("To securely delete the account, manually delete all secrets")
        print("owned by the account from Barbican.")
        return

    with SwiftService(options=options) as swift:
        try:
            # Check if container/object exists before starting to perform any
            # rotation. secdel_exists() raises an error if container or object
            # does not exist.
            if (secdel_exists(output_manager, swift, container, object) is
                    False):
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "secdel_exists() returned false, exiting")
                return
            logger.log(SECDEL_LOG_LEVEL_DEBUG,
                       "secdel_exists() ran successfully, continuing..")

            if root_uuid is None:
                # Start new secure deletion operation by creating a new
                # root encryption secret in Barbican
                barbican_mgr, barbican_ctxt = (
                    secdel_get_barbican_manager_and_ctxt(
                        output_manager, options, 'swiftclient.swiftclient_'
                        'key_manager.SwiftClientKeyManager'))
                # Retrieve a list of previously existing secrets that need to
                # be deleted after the key rotation completes.
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "calling secdel_create_barbican_secret()")
                root_uuid = secdel_create_barbican_secret(
                    output_manager, barbican_mgr, barbican_ctxt)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Created new root encryption secret with ID '%s'" %
                           root_uuid)
            else:
                # Continue interrupted secure deletion operation. Make sure
                # specified root encryption secret exists.
                # TODO: We could delegate this check to the swift cluster
                barbican_mgr, barbican_ctxt = (
                    secdel_get_barbican_manager_and_ctxt(
                        output_manager, options, 'swiftclient.swiftclient_'
                        'key_manager.SwiftClientKeyManager'))
                root_encryption_secret = secdel_get_barbican_secret(
                    output_manager, barbican_mgr, barbican_ctxt, root_uuid)
                if root_encryption_secret is None:
                    return
            logger.log(SECDEL_LOG_LEVEL_DEBUG, "Rekeying account")
            account_keys_before = secdel_list_keys(output_manager, swift)
            secdel_rekey(output_manager, swift)
            account_keys_after = secdel_list_keys(output_manager, swift)
            if object is not None:
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "Securely deleting object")
                container_keys_before = secdel_list_keys(
                    output_manager, swift, container)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start processing containers")
                list_res = secdel_list_account(output_manager, swift)
                for list_rest_part in list_res:
                    if list_rest_part["success"] is False:
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Error listing containers")
                        return
                    containers = list_rest_part['listing']
                    for c in containers:
                        name = c['name']
                        if name == container:
                            logger.log(
                                SECDEL_LOG_LEVEL_DEBUG,
                                "Rekeying parent container '%s'" % name)
                            secdel_rekey(output_manager, swift, name)
                        else:
                            logger.log(
                                SECDEL_LOG_LEVEL_DEBUG,
                                "Rewrapping other container '%s'" % name)
                            secdel_rewrap(output_manager, swift, name)
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "End processing containers")
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "Start processing objects")
                list_res = secdel_list_container(
                    output_manager, swift, container)
                for list_rest_part in list_res:
                    if list_rest_part["success"] is False:
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Error listing objects in %s" % container)
                        return
                    objects = list_rest_part['listing']
                    for o in objects:
                        name = o['name']
                        if name == object:
                            logger.log(
                                SECDEL_LOG_LEVEL_DEBUG,
                                "Not rewrapping object that is to be "
                                "deleted '%s'" % name)
                        else:
                            logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                       "Rewrapping other object '%s'" % name)
                            secdel_rewrap(
                                output_manager, swift, container, [name])
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "End processing objects")
                if not options['retain']:
                    logger.log(SECDEL_LOG_LEVEL_DEBUG, "Start deleting object")
                    _perform_delete(swift, args, output_manager, options)
                    logger.log(SECDEL_LOG_LEVEL_DEBUG, "End deleting object")
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start purging container keys")
                container_keys_after = secdel_list_keys(
                    output_manager, swift, container)
                for k in container_keys_after:
                    logger.log(SECDEL_LOG_LEVEL_DEBUG,
                               "Container key exists after rotation: %s" % k)
                for k in container_keys_before:
                    if k in container_keys_after:
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Purging container key '%s'" % k)
                        secdel_purge(output_manager, swift, k, container)
                    else:
                        logger.log(
                            SECDEL_LOG_LEVEL_DEBUG,
                            "Not purging new container key '%s" % k)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "End purging container keys")
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start purging account keys")
                for k in account_keys_before:
                    if k in account_keys_after:
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Purging account key '%s'" % k)
                        secdel_purge(output_manager, swift, k)
                    else:
                        logger.log(
                            SECDEL_LOG_LEVEL_DEBUG,
                            "Not purging new account key '%s" % k)
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "End purging account keys")
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "Start purging master keys")
                root_secrets = secdel_list_barbican_secrets(
                    output_manager, barbican_mgr, barbican_ctxt)
                for root_secret in root_secrets:
                    if root_secret == root_uuid:
                        logger.log(
                            SECDEL_LOG_LEVEL_DEBUG,
                            "Not purging newly created root secret '%s'" %
                            root_secret)
                    else:
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Purging existing root secret '%s'" %
                                   root_secret)
                        secdel_delete_barbican_secret(
                            output_manager, barbican_mgr, barbican_ctxt,
                            root_secret)
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "End purging master keys")
            elif container is not None:
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Securely deleting container")
                container_keys_before = secdel_list_keys(
                    output_manager, swift, container)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start processing containers")
                ...
                for list_rest_part in list_res:
                    ...
                    for c in containers:
                        ...
                        if name == container:
                            logger.log(
                                SECDEL_LOG_LEVEL_DEBUG,
                                "Not rewrapping container that is to be "
                                "deleted '%s'" % name)
                        else:
                            logger.log(
                                SECDEL_LOG_LEVEL_DEBUG,
                            ...
                            secdel_rewrap(output_manager, swift, name)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "End processing containers")
                if not options['retain']:
                    logger.log(SECDEL_LOG_LEVEL_DEBUG,
                               "Start deleting container")
                    _perform_delete(swift, args, output_manager, options)
                    logger.log(SECDEL_LOG_LEVEL_DEBUG,
                               "End deleting container")
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start purging account keys")
                for k in account_keys_before:
                    if k in account_keys_after:
                ...
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Error: Secure deletion of account not implemented")
                return
            return
...
st_rekey_options = '''[--root-uuid <uuid>]
'''

st_rekey_help = '''
Rekeys the account or container. This involves generating a new KEK for the
account or container, and rewrapping all child entities (containers or
objects). Containers or objects that have previously been deleted (but not
securely deleted) are effectively securely deleted after the rekey operation
has been performed.

Positional arguments:
  [<container>]           Name of container to rekey. If omitted, the account
                          is rekeyed.

Optional arguments:
  -u, --root-uuid <uuid>  Use the root secret specified by the given uuid for
                          completing the rekey operation. This argument can be
                          used to continue an interrupted rekey operation.
'''.strip('\n')


def st_rekey(parser, args, output_manager):
    parser.add_argument(
        '-H', '--header', action='append', dest='header',
        default=[],
        help='Adds a custom request header to use for securely deleting '
    ...
    if args is not None:
        ...
        if len(args) > 0:
            ...
            container = args[0]
        if len(args) > 1:
            print("An object cannot be rekeyed.")
            return 1
    else:
        logger.log(SECDEL_LOG_LEVEL_DEBUG, "args is None")
    ...
    with SwiftService(options=options) as swift:
        try:
            ...
            if ((container is not None) and
                    (secdel_exists(output_manager, swift, container) is
                     False)):
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "secdel_exists() returned false, exiting")
                return
            ...
            if root_uuid is None:
                ...
                barbican_mgr, barbican_ctxt = (
                    secdel_get_barbican_manager_and_ctxt(
                        output_manager, options, 'swiftclient.swiftclient_key_'
                        'manager.SwiftClientKeyManager'))
                root_encryption_secret = secdel_get_barbican_secret(
                    output_manager, barbican_mgr, barbican_ctxt, root_uuid)
                if root_encryption_secret is None:
                    return
            ...
            if container is not None:
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "Rekeying container")
                container_keys_before = secdel_list_keys(
                    output_manager, swift, container)
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start processing containers")
                ...
                for list_rest_part in list_res:
                    ...
                    for o in objects:
                        name = o['name']
                        logger.log(SECDEL_LOG_LEVEL_DEBUG,
                                   "Rewrapping object '%s'" % name)
                        secdel_rewrap(output_manager, swift, container, [name])
                logger.log(SECDEL_LOG_LEVEL_DEBUG, "End processing objects")
                logger.log(SECDEL_LOG_LEVEL_DEBUG,
                           "Start purging container keys")
                container_keys_after = secdel_list_keys(
...
Positional arguments:
  <subcommand>
    ...
    secdel               Securely delete a container or object.
    rekey                Rekey an account or container.
